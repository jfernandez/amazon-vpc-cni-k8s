...
func add(args *skel.CmdArgs, cniTypes typeswrapper.CNITYPES, grpcClient grpcwrapper.GRPC,
	...
	if r.PodVlanId != 0 {
		...
		// For dual-stack branch ENIs, populate the secondary IPv6 address if both IPv4 and IPv6 are available
		...
		if len(r.IPAllocationMetadata) > 0 {
			...
			if secondaryIPv6 != nil {
				log.Infof("Dual-stack branch ENI detected: primary=%v, secondary IPv6=%v", branchVethMetadata.IPAddress, secondaryIPv6)
		...
		// Add secondary IPv6 to CNI result for dual-stack branch ENIs
		if secondaryIPv6 != nil && err == nil {
			// Use the same container interface index as the primary IP
			containerInterfaceIndex := len(podInterfaces) - 1
...
func del(args *skel.CmdArgs, cniTypes typeswrapper.CNITYPES, grpcClient grpcwrapper.GRPC, rpcClient rpcwrapper.RPC,
	...
	if len(vethMetadata) > 0 {
		...
		if r.PodVlanId != 0 {
			...
			// For dual-stack branch ENIs, populate the secondary IPv6 address for cleanup
			branchVethMetadata := vethMetadata[0]
			if len(r.IPAllocationMetadata) > 0 {
				secondaryIPv6 := getSecondaryIPv6AddressFromIpAllocationMetadata(r.IPAllocationMetadata[0])
				if secondaryIPv6 != nil {
					log.Debugf("Dual-stack branch ENI teardown: primary=%v, secondary IPv6=%v", branchVethMetadata.IPAddress, secondaryIPv6)
					branchVethMetadata.IPv6Address = secondaryIPv6
				}
			}
...
// when both IPv4 and IPv6 are present in the allocation metadata (dual-stack branch ENI case)
